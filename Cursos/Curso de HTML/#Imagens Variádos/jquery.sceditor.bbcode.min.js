...
;(function ($, window, document) {
	...
	// SMFPACKS: Add mentions compatibility (body class)
	var _templates = {
		html:		'<!DOCTYPE html>' +
				'<html>' +
					'<head>' +
					...
					'</head>' +
					'<body class="advanced_editor" contenteditable="true" {spellcheck}></body>' +
				'</html>',
	...
	$.sceditor = function (el, options) {
		...
		init = function () {
			...
			if (!base.inSourceMode())
				$(".sceditor-button-source").addClass('active');
		};
		...
		initEditor = function () {
			...
			// SMFPACKS: Add mentions compatibility (ids)
			$sourceEditor  = $('<textarea id="smfpacks_advanced_editor"></textarea>').hide();
			$wysiwygEditor = $('<iframe id="iframe_smfpacks_advanced_editor" frameborder="0"></iframe>');
		...
		updateToolBar = function(disable) {
			...
			if (!inSourceMode)
				...
				$(".sceditor-button-source").removeClass('active');
		};
			...
			_dropDown: function(editor, caller, callback) {
				...
				if(!cmd._htmlCache)
				{
					...
					for (i = 0; i < colors.length; i++)
					{
						...
						html[htmlIndex++] = '<td><a href="#" class="sceditor-color-simple-option" style="width:15px; height: 15px; display: block; background-color: #'+colors[i]+'" data-color="#'+colors[i]+'"></a></td>';
					}
	...
	$.sceditor.plugins.bbcode.bbcodes = {
		...
		quote: {
			...
			html: function(element, attrs, content)
			{
				...
				var string = '&#91;quote';
				if (attrs['author'] || attrs['date'] || attrs['link'])
				{
					string += ' ';
